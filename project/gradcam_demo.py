...
# ensure local module import works when running this file
...
def save_index_html(folder, thumbnails, title="Grad-CAM results"):
    html = "<html><body><h2>{}</h2>".format(title)
    for t in thumbnails:
...
def main(args):
    ...
    if model is None:
        print("No repo model found; using torchvision EfficientNet-B0 fallback.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-path", default="", help="path to .pth/.pt model (optional)")
